...
async function _getFileData() {
  ...
  const url = `https://api.github.com/repos/${cfg.github_owner}/${cfg.github_repo}/contents/${cfg.data_file_path || 'data/posts.json'}`;
...
async function savePosts(posts) {
  ...
  const body = { message: 'update: posts.json', content };
...
function renderMarkdown(src) {
  ...
  function inline(text) {
    return text.split('`').map((p, i) => {
      if (i % 2 === 1) return `<code class="bg-gray-100 px-1 rounded text-sm font-mono">${escapeHtml(p)}</code>`;
      let s = escapeHtml(p);
      s = s.replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
      s = s.replace(/\*(.+?)\*/g, '<em>$1</em>');
      s = s.replace(/~~(.+?)~~/g, '<del>$1</del>');
      s = s.replace(/\[([^\]]+)\]\((https?:\/\/[^)]+|mailto:[^)]+)\)/g, '<a href="$2" class="text-primary underline" target="_blank" rel="noopener noreferrer">$1</a>');
      return s;
    }).join('');
  }
  ...
  for (const line of lines) {
    if (line.startsWith('```')) {
      if (inCode) {
        html += flushList() + `<pre class="bg-gray-50 border border-border rounded-lg p-4 overflow-x-auto my-3"><code class="text-sm font-mono">${escapeHtml(codeLines.join('\n'))}</code></pre>`;
    ...
    if (hm) {
      ...
      html += `<h${hm[1].length} class="${sz} font-bold my-3 text-on-surface">${inline(hm[2])}</h${hm[1].length}>`;
      continue;
    }
    if (/^-{3,}$/.test(line.trim())) { html += flushList() + '<hr class="my-4 border-border">'; continue; }
    if (line.startsWith('> ')) { html += flushList() + `<blockquote class="border-l-4 border-primary pl-4 my-2 text-grey-600 italic">${inline(line.slice(2))}</blockquote>`; continue; }
  ...
  html += flushList();
  if (inCode) html += `<pre class="bg-gray-50 border border-border rounded-lg p-4 overflow-x-auto my-3"><code class="text-sm font-mono">${escapeHtml(codeLines.join('\n'))}</code></pre>`;
  return html;
}
